...
REDRHEX_CFG = ArticulationCfg(
    ...
    spawn=sim_utils.UsdFileCfg(
        ...
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            ...
            linear_damping=0.2,                 # 線性阻尼：減緩直線運動，數值越大移動越慢
            angular_damping=0.3,                # 角阻尼：減緩旋轉運動
...
class RedrhexEnvCfg(DirectRLEnvCfg):
    ...
    # 10 個預定義的移動方向 [前後速度, 左右速度, 旋轉速度]
    # 這涵蓋了機器人可能需要的所有基本移動方式
    discrete_directions = [
        [0.4, 0.0, 0.0],     # 方向 0: 直走（向前）
        [-0.3, 0.0, 0.0],    # 方向 1: 後退
        [0.0, 0.3, 0.0],     # 方向 2: 側移（向左）
        [0.0, -0.3, 0.0],    # 方向 3: 側移（向右）
        [0.3, 0.2, 0.0],     # 方向 4: 斜走（左前方）
        [0.3, -0.2, 0.0],    # 方向 5: 斜走（右前方）
        [-0.2, 0.2, 0.0],    # 方向 6: 斜退（左後方）
        [-0.2, -0.2, 0.0],   # 方向 7: 斜退（右後方）
        [0.0, 0.0, 0.8],     # 方向 8: 原地旋轉（逆時針）
        [0.0, 0.0, -0.8],    # 方向 9: 原地旋轉（順時針）
    ]
    ...
    # =========================================================================
    # 步態（Gait）= 腿的運動模式和節奏
    # 就像人走路有固定的節拍，機器人的腿也需要協調的節奏
    
    # 基礎步態頻率（赫茲 Hz）
    # 1.0 Hz = 每秒完成 1 個完整的腿旋轉週期
    # 這決定了機器人「走」多快
    ...
    # 公式：角速度 = 2π × 頻率
    # 2π 弧度 = 360 度 = 轉一圈
    # 所以 1 Hz 對應 2π ≈ 6.28 rad/s
    base_gait_angular_vel = 2 * math.pi * 1.0
    
    # Tripod 組之間的相位差
    # π（180 度）表示兩組完全反相：
    # 當 Tripod A 著地時，Tripod B 在空中，反之亦然
    ...
    # 
    # 【什麼是獎勵函數？】
    # 獎勵函數就像訓練小狗時的「零食」和「懲罰」：
    # - 正值 = 獎勵：做對了給零食（鼓勵這個行為）
    # - 負值 = 懲罰：做錯了扣分（阻止這個行為）
    # 
    # AI 會學習最大化總獎勵，所以我們要仔細設計：
    # - 想要的行為 → 給正獎勵
    # - 不想要的行為 → 給負獎勵（懲罰）
    # 
    # 權重的絕對值越大，該項目越重要
    ...
    # 高度偏離懲罰：讓機器人保持正常站立高度
    rew_scale_base_height = -0.5
    
    # -------------------------------------------------------------------------
    # G3: 身體觸地懲罰（非常重要！防止摔倒）
    # -------------------------------------------------------------------------
    # 如果機身碰到地面 = 摔倒了，這是非常糟糕的！
    # 所以給一個很大的負值懲罰
    rew_scale_body_contact = -50.0
    
    # 是否在身體觸地時終止這一回合
    # True = 摔倒就 Game Over，要重新開始
    terminate_on_body_contact = True
    
    # 身體觸地的高度判斷閾值（公尺）
    # 當機身高度低於這個值，就判定為「摔倒」
    body_contact_height_threshold = 0.01
    
    # -------------------------------------------------------------------------
    # G4: 能耗與動作平滑懲罰（讓動作更自然）
    # -------------------------------------------------------------------------
    # 目標：減少不必要的能量消耗，讓動作更平滑
    
    # 力矩懲罰：使用太大的馬達力量會被懲罰
    # 鼓勵機器人用更省力的方式移動
    rew_scale_torque = -2.5e-5
    ...
    # 組內同步獎勵：同一組的三隻腳應該同步
    # 例如 Tripod A 的三隻腳要一起著地、一起離地
    rew_scale_tripod_sync = 1.0
    
    # 組間反相獎勵：兩組應該交替運動
    # 當 A 組著地時，B 組應該在空中
    rew_scale_tripod_antiphase = 1.0
    
    # -------------------------------------------------------------------------
    # G6: ABAD 關節使用策略
    # -------------------------------------------------------------------------
    # 目標：讓 ABAD 關節（外展/內收）在適當時機使用
    # 
    # 聰明使用的場景：需要轉彎或側移時
    # 浪費的場景：直走時卻亂擺腿
    
    # 聰明使用獎勵：在需要時正確使用 ABAD
    rew_scale_abad_smart_use = 1.5
    
    # 浪費懲罰：在不需要時亂用 ABAD
    rew_scale_abad_waste = -0.2
    
    # -------------------------------------------------------------------------
    # 存活獎勵
    # -------------------------------------------------------------------------
    # 設為 0 表示不給「只是活著」的獎勵
    # 這樣機器人不會學會「躺平不動」的偷懶策略
    # 必須積極移動才能獲得獎勵
    rew_scale_alive = 0.0
    
    # -------------------------------------------------------------------------
    # 舊版變數（為了相容性保留，但會被新系統覆蓋）
    # -------------------------------------------------------------------------
